...
import CreateAccessories from "@/form/CreateAccessories";
...
const Accessories = () => {
  ...
  const { data, isPending, refetch } = useQuery({
    queryKey: ["accessories", debouncedValue, pagination],
    queryFn: async () => {
      return accessoriesService.getAll(paramsTemp);
    },
  });
  ...
  const handleDelete = (id: string) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          const data = await accessoriesService.delete(id);
          if (data?.status) {
  ...
  return (
    <>
      ...
      <Drawer
        size="large"
        title={`${isEdit ? "Update" : "Create"} Accessories`}
        closeIcon={false}
        extra={
          <CircleX
            color={primaryColor}
