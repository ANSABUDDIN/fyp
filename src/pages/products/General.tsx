...
import CreateGeneral from "@/form/CreateGeneral";
...
const General = () => {
  ...
  const { data, isPending, refetch } = useQuery({
    queryKey: ["general", debouncedValue, pagination],
    queryFn: async () => {
      return generalProductService.getAll(paramsTemp);
    },
  });
  ...
  const handleDelete = (id: string) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          const data = await generalProductService.delete(id);
          if (data?.status) {
  ...
  return (
    <>
      ...
      <Drawer
        size="large"
        title={`${isEdit ? "Update" : "Create"} General`}
        closeIcon={false}
        extra={
          <CircleX
            color={primaryColor}
